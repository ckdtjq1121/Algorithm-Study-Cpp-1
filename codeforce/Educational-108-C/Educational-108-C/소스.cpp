...
int32_t main()
{
	...
	while (testCase--)
	{
		...
		for (int i = 0; i < n; i++)
		{
			...
			{
				if (j == 0)
					psum[i].push_back(st[i][j]);
				else
					psum[i].push_back(psum[i][j - 1] + st[i][j]);
			}
		}
		...
		for (int k = 1; k <= n; k++)
		{
			...
			for (int i = 0; i < n; i++)
			{
				...
				int rest = st[i].size() % k;
				//cout << i << " " << rest << "\n";
				if (rest >= 1)
					tmp += psum[i][st[i].size() - 1] - psum[i][rest - 1];
				else
					tmp += psum[i][st[i].size() - 1];
			}
